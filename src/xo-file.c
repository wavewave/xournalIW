...
#include <zlib.h>   // IW does not want to a gzipped file. 
#include <math.h>
#include <gdk/gdkx.h>
#include <X11/Xlib.h>
#include <locale.h>
...
// 2010.04.25. IW does change saving routine without using zlib. 
...
gboolean save_journal(const char *filename)
{
  ...
  for (pagelist = journal.pages; pagelist!=NULL; pagelist = pagelist->next) {
    ...
    fprintf(f, "<page width=\"%.2f\" height=\"%.2f\">\n", pg->width, pg->height);  // change
    fprintf(f, "<background type=\"%s\" ", bgtype_names[pg->bg->type]);  // change
    if (pg->bg->type == BG_SOLID) {
      fputs("color=\"",f);   // change
      if (pg->bg->color_no >= 0) fputs(bgcolor_names[pg->bg->color_no],f); // change
      else fprintf(f, "#%08x", pg->bg->color_rgba);  // change
      fprintf(f, "\" style=\"%s\" ", bgstyle_names[pg->bg->ruling]); // change
    }
      ...
      if (is_clone >= 0)
        fprintf(f, "domain=\"clone\" filename=\"%d\" ", is_clone); // change
      else {
        ...
        fprintf(f, "domain=\"%s\" filename=\"%s\" ",   // change
      ...
      if (!is_clone) {
        ...
        tmpstr = g_markup_escape_text(pg->bg->filename->s, -1);
        fprintf(f, "domain=\"%s\" filename=\"%s\" ", 
          file_domain_names[pg->bg->file_domain], tmpstr);
        g_free(tmpstr);
      }
      fprintf(f, "pageno=\"%d\" ", pg->bg->file_page_seq); // change
    }
    fprintf(f, "/>\n");  // change
    for (layerlist = pg->layers; layerlist!=NULL; layerlist = layerlist->next) {
      ...
      for (itemlist = layer->items; itemlist!=NULL; itemlist = itemlist->next) {
        ...
        if (item->type == ITEM_STROKE) {
          fprintf(f, "<stroke tool=\"%s\" color=\"",   // change
                          tool_names[item->brush.tool_type]);
          if (item->brush.color_no >= 0)
            fputs(color_names[item->brush.color_no], f);  //change
          else
            fprintf(f, "#%08x", item->brush.color_rgba);  // change
          fprintf(f, "\" width=\"%.2f", item->brush.thickness); // change
          if (item->brush.variable_width)
            for (i=0;i<item->path->num_points-1;i++)
              fprintf(f, " %.2f", item->widths[i]);  // change
          fprintf(f, "\">\n");  // change
          for (i=0;i<2*item->path->num_points;i++)
            fprintf(f, "%.2f ", item->path->coords[i]);  // change
          fprintf(f, "\n</stroke>\n");  // change
        }
        if (item->type == ITEM_TEXT) {
          tmpstr = g_markup_escape_text(item->font_name, -1);
          fprintf(f, "<text font=\"%s\" size=\"%.2f\" x=\"%.2f\" y=\"%.2f\" color=\"",  // change
